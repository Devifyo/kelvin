...
class PrivacyPageSettings extends Component
{
    public $contentId;
    public $tab = 'header';
    ...
    {
        $record = PrivacyPageContent::first();
        if ($record) {
    ...
    public function save()
    {
        ...
        if ($this->contentId) {
            PrivacyPageContent::where('id', $this->contentId)->update($data);
        } else {
            $record = PrivacyPageContent::create($data);
            $this->contentId = $record->id;
        }

        session()->flash('success', 'Privacy Policy page updated successfully.');
    }
    ...
    {
        return view('livewire.admin.privacy-page-settings')->title('Privacy Policy Page');
    }
}
