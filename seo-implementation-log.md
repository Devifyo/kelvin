...
| [resources/views/layouts/app.blade.php](resources/views/layouts/app.blade.php) (2nd pass) | Made `og:image` always emit with a three-tier fallback: page `@section('og_image')` → `seo_og_image` AppSetting → layout default (headshot). Added `og:locale = en_US`, `og:image:alt`, `twitter:image:alt`. Removed the conditional gate that suppressed `og:image` when AppSetting was empty. Added `Person.image` (`ImageObject`) for knowledge-graph thumbnail. |
| [resources/views/landing-pages/blog-show.blade.php](resources/views/landing-pages/blog-show.blade.php) (2nd pass) | Wrapped `og_image` with `url()` so featured-image URLs are absolute (was relative `/storage/...`). Added `og_image_alt` from post title. Replaced hardcoded `https://kevin.devifyo.cloud/img/...` (dev domain) with `asset()`-resolved URL on the author avatar and bio image. |
| [resources/views/landing-pages/training-show.blade.php](resources/views/landing-pages/training-show.blade.php) (2nd pass) | Added `og_image_alt` from service title. |
| [resources/views/landing-pages/about.blade.php](resources/views/landing-pages/about.blade.php) (2nd pass) | Replaced bare `<img>` with `<figure>` + `<figcaption>` + `ImageObject` microdata. Improved `alt` to "Portrait of Dr. Kevin Thompson, Ph.D. — Agile hardware development consultant". Added `decoding="async"`, `itemprop="contentUrl"`. |
| [resources/views/landing-pages/training.blade.php](resources/views/landing-pages/training.blade.php) (2nd pass) | Replaced 5 hardcoded course cards with a `@forelse($trainingClasses)` loop using real Service records. Switched single-column stack to a 2-column responsive grid. Updated `ItemList` JSON-LD to include `audience`. |
| [app/Services/SeoGenerator.php](app/Services/SeoGenerator.php) (2nd pass) | Sitemap now includes the `xmlns:image="http://www.google.com/schemas/sitemap-image/1.1"` namespace and emits `<image:image>` children for the homepage + about-page headshot, every Post's `featured_image`, and every active Service's `featured_image`. Verified: 18 URLs, 5 image entries. |
| [seo.md](seo.md) (new) | **Single source of truth** for the SEO architecture. Documents layout → page → model → AppSetting → SeoGenerator chain, every `seo_*` AppSetting key, the Eloquent hook auto-regen flow, the `seo:generate` command, image SEO patterns, server-level config, and recipes for adding new pages/models/schema. |
